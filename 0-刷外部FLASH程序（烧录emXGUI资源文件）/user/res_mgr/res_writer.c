...
extern uint8_t SPI_FLASH_BufferWrite(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite);
extern uint8_t SPI_FLASH_BufferRead(uint8_t* pBuffer, uint32_t ReadAddr, uint16_t NumByteToRead);
/**
  * @brief  检查是否要忽略文件夹或文件
...
{  
  ...
  for(i=0;1;i++)
  {
    ...
    while(result == FR_OK) 
    {
      ...
      state = SPI_FLASH_BufferWrite(tempbuf,write_addr,bw);  //拷贝数据到外部flash上   
      if(state != 1)
      {
  ...
  for(i=0;1;i++)
  {
    ...
    BURN_INFO("内容大小：%d ",dir.size);
      ...
      while(result == FR_OK) 
      {
        ...
        state = SPI_FLASH_BufferRead(flash_buf,read_addr,bw);  //从FLASH中读取数据
        if(state != 1)
        {
