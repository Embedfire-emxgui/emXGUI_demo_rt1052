...
//			TextOut(hdc,rc.x,rc.y,L"Chrome加速器",-1); 
      wnd=CreateWindow(BUTTON,L"Chrom加速器",TBS_FLAT|WS_DISABLED|WS_TRANSPARENT|WS_OWNERDRAW|WS_VISIBLE,rc.x,rc.y,rc.w+40,rc.h,hwnd,ID_INFO,NULL,NULL); //创建一个文字框.
			...
			wnd=CreateWindow(BUTTON,L"随机图案",TBS_FLAT|WS_OWNERDRAW|WS_TRANSPARENT|WS_VISIBLE,0,0,200,35,hwnd,ID_TITLE,NULL,NULL); //创建一个文字框.
			SetWindowFont(wnd,defaultFont); //设置控件窗口字体.
      
      /* Chrom-ART 激活 */      
      wnd=CreateWindow(BUTTON,L"图形加速器 激活",TBS_FLAT|WS_OWNERDRAW|WS_TRANSPARENT|WS_VISIBLE,400,0,200,35,hwnd,ID_ART_ACTIVE,NULL,NULL); //创建一个文字框.
      SetWindowFont(wnd,defaultFont); //设置控件窗口字体.
      g_dma2d_en = TRUE;
