...
void (*MapperInit)();
//Write to Mapper 
void (*MapperWrite)( NWORD wAddr, NBYTE byData );
//Write to SRAM 
void (*MapperSram)( NWORD wAddr, NBYTE byData );
//Write to Apu 
void (*MapperApu)( NWORD wAddr, NBYTE byData );
//Read from Apu 
NBYTE (*MapperReadApu)( NWORD wAddr );
//Callback at VSync 
void (*MapperVSync)();
//Callback at HSync 
void (*MapperHSync)();
//Callback at PPU read/write 
void (*MapperPPU)( NWORD wAddr );
//Callback at Rendering Screen 1:BG, 0:Sprite
void (*MapperRenderScreen)( NBYTE byMode );
 ...
 */
  ...
  if ( NES->FrameCnt == 0 &&
       NES->PPU_ScanTable[ NES->PPU_Scanline ] == SCAN_ON_SCREEN )
  {
			if((NES->PPU_Scanline>=8)&&(NES->PPU_Scanline<232))
			{
				InfoNES_DrawLine();
				LCD_DrawLine();
			}
...
//       }
    ...
    case SCAN_VBLANK_START:             //FrameSkip
      NES->FrameCnt = ( NES->FrameCnt >= FRAME_SKIP ) ? 0 : NES->FrameCnt + 1;
      // Set a V-Blank flag
 ...
 */
  ...
  int nSprData;
  NBYTE bySprCol;
  NBYTE pSprBuf[ NES_DISP_WIDTH + 7 ];
  ...
  pPoint = NES->Linebuf[0];//&WorkFrame[ PPU_Scanline * NES_DISP_WIDTH ];
  ...
  else
  {
    ...
    {
      NWORD *pPointTop;
      pPointTop = NES->Linebuf[0];//&WorkFrame[ PPU_Scanline * NES_DISP_WIDTH ];
  ...
  if ( NES->PPU_R1 & R1_SHOW_SP )
  {
    ...
    {
      nSprData = pSprBuf[ nX ];
      if ( nSprData  && ( nSprData & 0x80 || pPoint[ nX ] & 0x0020 ) )
      {
 ...
 */
  ...
  if ( NES->PPU_R0 & R0_SP_SIZE )
  {
    ...
    {
      pdwChrData = (NDWORD *)( ChrBuf + 256 * 64 + ( ( NES->SPRRAM[ SPR_CHR ] & 0xfe ) << 6 ) + nYBit );
    }
    else
    {
      pdwChrData = (NDWORD * )( ChrBuf + ( ( NES->SPRRAM[ SPR_CHR ] & 0xfe ) << 6 ) + nYBit );
    } 
  }
  else
  {
    // Sprite size 8x8
    pdwChrData = (NDWORD *)( NES->PPU_SP_Base + ( NES->SPRRAM[ SPR_CHR ] << 6 ) + nYBit );
  }
