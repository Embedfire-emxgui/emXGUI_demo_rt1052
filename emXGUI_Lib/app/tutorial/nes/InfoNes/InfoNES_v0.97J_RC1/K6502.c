...
/*-------------------------------------------------------------------*/
 ...
 NWORD K6502_Read8(void)
{
  return K6502_Read( CPU->PC++ );
}
...
// Zero Page,X
#define AA_ZPX   (NBYTE)( K6502_Read( CPU->PC++ ) + CPU->X )
// Zero Page,Y
#define AA_ZPY   (NBYTE)( K6502_Read( CPU->PC++ ) + CPU->Y )
// Absolute
#define AA_ABS   ( K6502_Read8() | (NWORD)K6502_Read( CPU->PC++ ) << 8 )
// Absolute2 ( PC-- )
#define AA_ABS2  ( K6502_Read8() | (NWORD)K6502_Read( CPU->PC ) << 8 )
// Absolute,X
...
#define CMP(a)  wD0 = (NWORD)CPU->A - (a); RSTF( FLAG_N | FLAG_Z | FLAG_C ); SETF( CPU->g_byTestTable[ wD0 & 0xff ] | ( wD0 < 0x100 ? FLAG_C : 0 ) );
#define CPX(a)  wD0 = (NWORD)CPU->X - (a); RSTF( FLAG_N | FLAG_Z | FLAG_C ); SETF( CPU->g_byTestTable[ wD0 & 0xff ] | ( wD0 < 0x100 ? FLAG_C : 0 ) );
#define CPY(a)  wD0 = (NWORD)CPU->Y - (a); RSTF( FLAG_N | FLAG_Z | FLAG_C ); SETF( CPU->g_byTestTable[ wD0 & 0xff ] | ( wD0 < 0x100 ? FLAG_C : 0 ) );
...
#define BRA(a) { \
  ...
  { \
	...
	CPU->PC += ( ( byD0 & 0x80 ) ? ( 0xFF00 | (NWORD)byD0 ) : (NWORD)byD0 ); \
...
// Data
// Absolute,X
static  NBYTE K6502_ReadAbsX(){ NWORD wA0, wA1; wA0 = AA_ABS; wA1 = wA0 + CPU->X; CLK( ( wA0 & 0x0100 ) != ( wA1 & 0x0100 ) ); return K6502_Read( wA1 ); };
// Absolute,Y
static  NBYTE K6502_ReadAbsY(){ NWORD wA0, wA1; wA0 = AA_ABS; wA1 = wA0 + CPU->Y; CLK( ( wA0 & 0x0100 ) != ( wA1 & 0x0100 ) ); return K6502_Read( wA1 ); };
// (Indirect),Y
static  NBYTE K6502_ReadIY(){ NWORD wA0, wA1; wA0 = K6502_ReadZpW( K6502_Read( CPU->PC++ ) ); wA1 = wA0 + CPU->Y; CLK( ( wA0 & 0x0100 ) != ( wA1 & 0x0100 ) ); return K6502_Read( wA1 ); };
